...
Config.getAll().then(function (config) {
  ...
  let scripts = [
    ...
    [
      ...
      config.isPluginEnabled && (config.enabledForRecaptchaV2 || config.enabledForInvisibleRecaptchaV2 || config.enabledForRecaptchaV3) && !isBlocked,
    ],
...
function createSolverButton(captchaType, widgetId, config) {
  let button = $(`
        <div class="captcha-solver captcha-solver_${
          config.buttonPosition
        }" data-state="ready" data-captcha-type="${captchaType}" data-widget-id="${widgetId}">
